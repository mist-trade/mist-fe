...
### Requirement: Backtest Visualization Shall Use Dual-Request Same-Params At Second Precision

The system SHALL enforce dual-request same-params at second precision: frontend at `/k` and `/backtests` for K-line and Chan geometry rendering SHALL use the same query concurrent dual requests and SHALL preserve HH:MM:SS precision.
...
### Requirement: Dual-Request Time Axis Alignment Invariant Shall Hold

The system SHALL enforce that when `TradingViewChart` merges rendering with `{k, commands}`, geometry times SHALL hit in the K sequence, otherwise it SHALL be treated as contract violation.
